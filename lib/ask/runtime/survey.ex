defmodule Ask.Runtime.Survey do
  ...
  def mask_phone_number(%Respondent{} = respondent, {:reply, response}) do
    ...
    masked_response =
      if String.length(pii) != 0 && contains_phone_number(response, pii) do
  ...
  def handle_session_step(
        {:ok, %{respondent: respondent} = session, reply, timeout},
        now,
        persist
      ) do
    ...
    updated_respondent =
      respondent_updates(:ok, respondent, session, Reply.disposition(reply), timeout_at, persist)
  ...
  def handle_session_step({:hangup, session, reply, timeout, respondent}, _, persist) do
    timeout_at =
      Respondent.next_actual_timeout(respondent, timeout, SystemTime.time().now, persist)
  ...
  def handle_session_step({:end, reply, respondent}, _, persist) do
    ...
    updated_respondent =
      respondent_updates(:end, respondent, new_disposition, persist)
    ...
    if persist && session && new_disposition != old_disposition &&
         new_disposition != reply_disposition do
      mode = session.current_mode |> SessionMode.mode()

      Session.log_disposition_changed(
        updated_respondent,
  ...
  def handle_session_step(
        {:rejected, %{respondent: respondent} = session, reply, timeout},
        _,
        persist
      ) do
    timeout_at =
  ...
  def handle_session_step({:stopped, reply, respondent}, _, persist) do
    updated_respondent =
      respondent_updates(:stopped, respondent, Reply.disposition(reply), persist)

    updated_respondent =
      if(disposition_changed?(respondent, updated_respondent)) do
        disposition_changed(
          updated_respondent,
          Session.load_respondent_session(respondent, persist),
  ...
  defp disposition_changed?(original_respondent, updated_respondent),
    do: original_respondent.disposition != updated_respondent.disposition

  defp disposition_changed(respondent, session, old_disposition, persist) do
    if(persist) do
      mode =
        if session && session.current_mode do
  ...
  defp respondent_updates(:end, respondent, disposition, persist) do
    changes = %{
      ...
      session: nil,
      completed_at: SystemTime.time().now,
      timeout_at: nil
    }
  ...
  defp respondent_updates(:ok, respondent, session, disposition, timeout_at, persist) do
    if disposition do
      ...
      changes =
        respondent_updates_with_disposition(
          respondent,
          session,
          timeout_at,
          intended_changes,
  ...
  defp no_disposition_changes(respondent, session, timeout_at, persist) do
    ...
    %{
      ...
      session: session_dump,
      timeout_at: timeout_at,
      language: session.flow.language,
  ...
  defp session_mode(respondent, session, mode) do
    if mode == session.current_mode |> SessionMode.mode() do
      ...
      channel_group =
        (group |> Repo.preload(respondent_group_channels: :channel)).respondent_group_channels
    ...
    transaction_result =
      Repo.transaction(fn ->
        try do
          ...
          # if don't want to persist, makes no sense to log the response
          session_step = Session.sync_step(session, reply, session_mode, persist, persist)
          ...
          e in Ecto.StaleEntryError ->
            Logger.error(
              e,
              __STACKTRACE__,
              "Error on sync step internal. Rolling back transaction"
            )
          ...
          e ->
            ...
            respondent =
              if persist,
                do: Repo.get(Respondent, session.respondent.id),
                else: session.respondent

            Logger.error(
              e,
              __STACKTRACE__,
              "Error occurred while processing sync step (survey_id: #{respondent.survey_id}, respondent_id: #{
                respondent.id
              })"
            )
  ...
  defp respondent_updates_with_disposition(
         respondent,
         session,
         timeout,
         %{disposition: disposition} = changes,
         persist
       ) do
    old_disposition = respondent.disposition
  ...
  defp handle_next_action(next_action, respondent_id, persist) do
    if persist do
      ...
      session =
        if respondent.session, do: Session.load(respondent.session), else: respondent.session
