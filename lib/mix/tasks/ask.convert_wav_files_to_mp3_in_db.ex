defmodule Mix.Tasks.Ask.ConvertWavFilesToMp3InDb do
  ...
  defp convert_audio(audio) do
    ...
    case Sox.convert("wav", path, "mp3") do
      {:ok, mp3} -> case Repo.update(Audio.changeset(audio, %{"data" => mp3, "filename" => mp3_filename})) do
                      {:ok, _} -> Mix.shell.info("Converted #{audio.id} #{audio.filename}")
                      {:error, changeset} -> Mix.shell.error("Failed inserting #{audio.id} #{audio.filename}: #{inspect changeset}")
                    end
      {:error, err} -> Mix.shell.error("Failed converting #{audio.id} #{audio.filename}: #{err}")
    end
  end
