defmodule AskWeb.IntegrationController do
  ...
  def create(conn, %{
    ...
    case create_integration(conn, integration_params, survey) do
      {:ok, integration} ->
        ...
        |> put_resp_header(
          "location",
          project_survey_integration_path(conn, :index, survey.project_id, survey.id)
        )
  ...
  defp create_integration(
         conn,
         params = %{"name" => _name, "uri" => uri, "auth_token" => _auth_token},
         survey
       ) do
    changeset =
      FloipEndpoint.changeset(%FloipEndpoint{}, params)
      |> put_assoc(:survey, survey)

    endpoint = apply_changes(changeset)

    case FloipPusher.create_package(
           survey,
           endpoint,
      ...
      error ->
        Logger.error(
          "Error creating integration: survey: #{survey.id}, endpoint_uri: #{uri}, response from target service: #{
            inspect(error)
          }"
        )
