defmodule AskWeb.QuestionnaireSimulationController do
  ...
  def start(conn, %{"project_id" => project_id, "questionnaire_id" => id}) do
    ...
    with {:ok, questionnaire} <- load_questionnaire(project, id),
         {:ok, simulation_response} <-
           QuestionnaireSimulator.start_simulation(project, questionnaire, mode) do
  ...
  def sync(conn, %{"respondent_id" => respondent_id, "response" => response, "mode" => mode}) do
    with {:ok, simulation_response} <-
           QuestionnaireSimulator.process_respondent_response(respondent_id, response, mode),
         do: render(conn, "simulation.json", simulation: simulation_response)
  end
