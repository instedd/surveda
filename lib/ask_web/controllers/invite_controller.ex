defmodule AskWeb.InviteController do
  use AskWeb, :api_controller

  alias Ask.{Project, ProjectMembership, Invite, Logger, ActivityLog}
  alias Ecto.Multi
  import Ecto.Query
        ...
        if invite.code == code do
          if(invite.level == level) do
            ...
            update_multi =
              ...
              |> Multi.insert(
                :insert_log,
                ActivityLog.edit_invite(project, conn, email, invite.level, level)
              )
              |> Repo.transaction()
  ...
  def show(conn, %{"code" => code}) do
    ...
    if !invite do
      ...
      project_membership =
        ProjectMembership |> Repo.get_by(user_id: user.id, project_id: project.id)
  ...
  def perform_update(conn, email, project, new_level) do
    ...
    multi =
      ...
      |> Multi.insert(
        :insert,
        ActivityLog.edit_invite(project, conn, email, invite.level, new_level)
      )
      |> Repo.transaction()
