defmodule AskWeb.PageController do
  ...
  plug Guisso.SSO, session_controller: AskWeb.Coherence.SessionController

  def index(conn, params = %{"path" => path}) do
    explicit = params["explicit"]

    if params["locale"] do
      Gettext.put_locale(AskWeb.Gettext, params["locale"])
    end
    ...
    case {path, user, explicit} do
      {_, _, "true"} ->
        conn
        |> render("landing.html",
          current_locale: current_locale_description(Gettext.get_locale(AskWeb.Gettext))
        )
