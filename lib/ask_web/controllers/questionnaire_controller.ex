defmodule AskWeb.QuestionnaireController do
  ...
  action_fallback AskWeb.FallbackController
  ...
  def show(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    with {:ok, questionnaire} <- load_questionnaire(project, id),
         do: render(conn, "show.json", questionnaire: questionnaire)
  end
  ...
  def update(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    multi =
      ...
      |> Multi.update(:questionnaire, changeset,
        force: Map.has_key?(changeset.changes, :questionnaires)
      )
  ...
  def delete(conn, %{"project_id" => project_id, "id" => id}) do
    ...
    changeset =
      load_questionnaire_not_snapshot(project.id, id)
    ...
    case multi do
      {:ok, _} ->
        ...
        surveys =
          from(s in Survey, join: sq in SurveyQuestionnaire, on: sq.questionnaire_id == ^id)
          |> Repo.all()
  ...
  def export_zip(conn, %{"project_id" => project_id, "questionnaire_id" => id}) do
    ...
    underscored_name =
      ...
      # replace spaces with underscore (also check for continuos spaces)
    ...
    conn =
      ...
      |> put_resp_header(
        "content-disposition",
        "attachment; filename*=UTF-8''#{questionnaire.id}-#{URI.encode(underscored_name)}.zip"
      )
      |> send_chunked(200)
  ...
  def import_zip(conn, %{"project_id" => project_id, "questionnaire_id" => id, "file" => file}) do
    ...
    |> Enum.each(fn %{
      ...
      unless Audio |> Repo.get_by(uuid: uuid) do
        ...
        %Audio{uuid: uuid, data: data, filename: original_filename, source: source}
        |> Repo.insert!()
      end
    end)
