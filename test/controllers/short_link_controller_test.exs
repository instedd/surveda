defmodule Ask.ShortLinkControllerTest do
  ...
  test "render surveys if the link specifies that endpoint even if there is no current user", %{conn: conn, user: user} do
    ...
    assert json_response(conn, 200)["data"] == [
      %{"cutoff" => survey.cutoff, "id" => survey.id, "mode" => survey.mode, "name" => survey.name, "description" => nil, "project_id" => project.id, "state" => "not_ready", "locked" => false, "exit_code" => nil, "exit_message" => nil, "schedule" => %{"blocked_days" => [], "day_of_week" => %{"fri" => true, "mon" => true, "sat" => true, "sun" => true, "thu" => true, "tue" => true, "wed" => true}, "end_time" => "23:59:59", "start_time" => "00:00:00", "timezone" => "Etc/UTC"}, "next_schedule_time" => nil, "down_channels" => [], "started_at" => nil, "updated_at" => DateTime.to_iso8601(survey.updated_at)}
    ]
  end
end
