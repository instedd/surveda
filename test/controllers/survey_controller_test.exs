defmodule Ask.SurveyControllerTest do
  ...
  describe "changes the survey state when needed" do
    ...
    test "updates state when removing cutoff", %{conn: conn, user: user} do
      [project, questionnaire, channel] = prepare_for_state_update(user)
    ...
    test "does not update state when adding questionnaire if missing cutoff", %{conn: conn, user: user} do
      [project, questionnaire, channel] = prepare_for_state_update(user)
