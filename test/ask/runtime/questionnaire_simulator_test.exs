defmodule Ask.Runtime.QuestionnaireSimulatorTest do
  use Ask.DataCase
  use Ask.DummySteps
  import Ask.Factory
  ...
  def process_respondent_response(respondent_id, response, mode \\ "sms") do
    {:ok, simulation_step} =
      QuestionnaireSimulator.process_respondent_response(respondent_id, response, mode)
  ...
  defp get_last_simulation_response(respondent_id) do
    {:ok, simulation_response} =
      QuestionnaireMobileWebSimulator.get_last_simulation_response(respondent_id)
  ...
  describe "Mobile Web - last_simulation_response field" do
    test "should keep the last simulation response", %{start_simulation: start_simulation} do
      ...
      %{respondent_id: respondent_id} =
        first_simulation_response = start_simulation.(quiz, "mobileweb")
    ...
    test "non-present simulation should return a SimulationStep with status: expired" do
      ...
      assert Ask.Simulation.Status.expired() ==
               Map.get(last_simulation_response, :simulation_status)

      assert respondent_id == Map.get(last_simulation_response, :respondent_id)
    end

    test "SMS simulation should response invalid_simulation", %{
  ...
  describe "base cases" do
    ...
    test "with partial flag", %{start_simulation: start_simulation} do
      ...
      Enum.map(["sms", "mobileweb"], fn mode ->
        ...
        %{respondent_id: respondent_id, disposition: disposition, simulation_status: status} =
          simulation
  ...
  describe "SMS - simulation messages_history field" do
    test "should maintain all respondent responses even if aren't valid", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(@dummy_steps)
    ...
    test "should include all the questions and responses", %{start_simulation: start_simulation} do
      ...
      # 1 is a yes response
      process_respondent_response(respondent_id, "1", "sms")
      # Y is a yes response
      process_respondent_response(respondent_id, "Y", "sms")
      # numeric response
      process_respondent_response(respondent_id, "7", "sms")

      # numeric response
      %{messages_history: messages_history} =
        process_respondent_response(respondent_id, "4", "sms")
  ...
  describe "simulation submissions field" do
    ...
    test "Mobile Web explanation steps behave differently", %{start_simulation: start_simulation} do
      ...
      # The simulation is started (the user asked for a simulation from the questionnaire screen)
      %{respondent_id: respondent_id, submissions: submissions} =
        start_simulation.(quiz, "mobileweb")
    ...
    test "should indicate as response the valid-parsed responses", %{
      start_simulation: start_simulation
    } do
      steps = @dummy_steps
      first = hd(steps)
    ...
    test "should not include the non-valid responses (since the step is not completed)", %{
      start_simulation: start_simulation
    } do
      steps = @dummy_steps
      quiz = questionnaire_with_steps(steps)
    ...
    test "should include all the responses, having or not the quiz a thank-you-message", %{
      start_simulation: start_simulation
    } do
      steps = @dummy_steps
  ...
  test "process_respondent_response of non-present simulation should return a SimulationStep with status: expired" do
    ...
    Enum.map(["sms", "mobileweb"], fn mode ->
      %{simulation_status: status, respondent_id: rid} =
        process_respondent_response(respondent_id, "No", mode)
  ...
  test "the simulator supports questionnaires with multiple sections", %{
    start_simulation: start_simulation
  } do
  ...
  describe "simulator responses invalid_simulation" do
    test "when start_simulation with ivr mode", %{project: project} do
      ...
      assert {:error, :invalid_simulation} ==
               QuestionnaireSimulator.start_simulation(project, quiz, "ivr")
    end

    test "when start_simulation with sms/mobileweb mode but questionnaire doesn't have it", %{
      project: project
    } do
      quiz =
      ...
      Enum.map(["sms", "mobileweb"], fn mode ->
        assert {:error, :invalid_simulation} ==
                 QuestionnaireSimulator.start_simulation(project, quiz, mode)
      end)
    end
  end

  describe "SMS - stop messages ends the simulation" do
    test "if stop message on contacted disposition, then final disposition is 'refused'", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(@dummy_steps)

      %{respondent_id: respondent_id, disposition: starting_disposition} =
        start_simulation.(quiz, "sms")

      assert :contacted == starting_disposition

      %{simulation_status: status, disposition: disposition} =
        process_respondent_response(respondent_id, "Stop", "sms")

      assert Simulation.Status.ended() == status
      assert :refused == disposition
    end

    test "if stop message on started disposition, then final disposition is 'breakoff'", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(@dummy_steps)
      %{respondent_id: respondent_id} = start_simulation.(quiz, "sms")
    ...
    test "if stop message on interim-partial disposition, then final disposition is 'breakoff'",
         %{start_simulation: start_simulation} do
      quiz = questionnaire_with_steps(SimulatorQuestionnaireSteps.with_interim_partial_flag())
      %{respondent_id: respondent_id} = start_simulation.(quiz, "sms")
      process_respondent_response(respondent_id, "No", "sms")

      %{disposition: previous_disposition} =
        process_respondent_response(respondent_id, "Yes", "sms")
  ...
  describe "questionnaire with relevant steps" do
    ...
    test "once the respondent reaches 'interim partial', simulation should return such disposition until completes the survey",
         %{start_simulation: start_simulation} do
      steps = QuestionnaireRelevantSteps.all_relevant_steps()
      ...
      Enum.map(["sms", "mobileweb"], fn mode ->
        ...
        %{disposition: disposition, simulation_status: status} =
          process_respondent_response(respondent_id, "4", mode)
    ...
    test "if respondent answer min_relevant_steps, even of different sections, disposition should be 'interim partial'",
         %{start_simulation: start_simulation} do
      ...
      Enum.map(["sms", "mobileweb"], fn mode ->
        ...
        # Second relevant, in different section
        %{disposition: disposition} = process_respondent_response(respondent_id, "No", mode)
        assert :"interim partial" == disposition
      end)
    end
  ...
  describe "questionnaire field" do
    ...
    test "should not be included in process_respondent_response", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(@dummy_steps)
    ...
    test "if quiz doesn't have sections, steps should be in the same order", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(@dummy_steps)
    ...
    test "if quiz has sections but not randomized, steps should be in the same order", %{
      start_simulation: start_simulation
    } do
      quiz = questionnaire_with_steps(SimulatorQuestionnaireSteps.two_sections_dummy_steps())
    ...
    test "if quiz has randomized sections, steps should be in the order it will be send to respondent",
         %{start_simulation: start_simulation} do
      Enum.map(["sms", "mobileweb"], fn mode ->
        quiz =
          questionnaire_with_steps(
            SimulatorQuestionnaireSteps.four_sections_randomized_dummy_steps()
          )
  ...
  defp randomized_section_order(quiz, start_simulation, mode) do
    ...
    %{section_order: section_order} =
      Ask.Runtime.QuestionnaireSimulatorStore.get_respondent_simulation(respondent_id)
  ...
  defp assert_dummy_steps(start_simulation, quiz, mode) do
    ...
    [first, second, third, fourth] =
      quiz |> Questionnaire.all_steps() |> Enum.map(fn step -> step["id"] end)
    ...
    %{disposition: disposition, current_step: current_step} =
      simulation = process_respondent_response(respondent_id, "No", mode)
    ...
    %{disposition: disposition, current_step: current_step} =
      simulation = process_respondent_response(respondent_id, "7", mode)

    assert :started == disposition
    on_sms_assert_last_message(simulation, "What's the number of this question??", mode)
    assert current_step == fourth

    %{disposition: disposition, simulation_status: status, current_step: current_step} =
      simulation = process_respondent_response(respondent_id, "4", mode)
  ...
  defp expected_submission(step, response),
    do: %{step_id: step["id"], response: response, step_name: submission_step_name(step)}

  defp expected_submission(step),
    do: %{step_id: step["id"], step_name: submission_step_name(step)}
...
defmodule SimulatorQuestionnaireSteps do
  ...
  def one_section_dummy_steps,
    do: [section(id: "Section1", title: "First Section", randomize: false, steps: @dummy_steps)]
  ...
  def two_sections_dummy_steps do
    ...
    [
      section(
        ...
        title: "First Section",
        randomize: false,
        steps: steps |> Enum.take(2)
      ),
      section(
        id: "Section2",
        title: "Second Section",
        randomize: false,
        steps: steps |> Enum.drop(2)
      )
    ]
  end
  ...
  def with_explanation_first_step,
    do: [
      explanation_step(
  ...
  def only_ivr_steps,
    do: [
      multiple_choice_step(
        id: Ecto.UUID.generate(),
        title: "Do you smoke?",
        prompt: prompt(ivr: tts_prompt("Do you smoke? Press 8 for YES, 9 for NO")),
        store: "Smokes",
        choices: [
      ...
      multiple_choice_step(
        id: Ecto.UUID.generate(),
        title: "Do you exercise",
        prompt: prompt(ivr: tts_prompt("Do you exercise? Press 1 for YES, 2 for NO")),
        store: "Exercises",
        choices: [
