defmodule AskWeb.QuestionnaireSimulationControllerTest do
  ...
  describe "start:" do
    ...
    test "renders json with the started SMS simulation", %{conn: conn, user: user} do
      ...
      conn =
        post conn,
             project_questionnaire_questionnaires_start_simulation_path(
               conn,
               :start,
    ...
    test "renders json with submissions if questionnaire starts with an explanation step", %{
      conn: conn,
      user: user
    } do
      project = create_project_for_user(user)
      ...
      explanation_step =
        explanation_step(
          id: "1",
          title: "Explanation",
          prompt: prompt(sms: sms_prompt("Welcome to the survey")),
          skip_logic: nil
        )
  ...
  describe "sync:" do
    ...
    test "renders json for started SMS simulation", %{conn: conn, user: user} do
      ...
      conn =
        post conn,
             project_questionnaire_questionnaires_sync_simulation_path(
               conn,
               :sync,
               questionnaire.project,
               questionnaire
