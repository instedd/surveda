defmodule AskWeb.QuestionnaireControllerTest do
  use AskWeb.ConnCase
  use Ask.DummySteps
  use Ask.TestHelpers
  ...
  describe "index:" do
    ...
    test "filters archived", %{conn: conn, user: user} do
      ...
      conn =
        get(conn, project_questionnaire_path(conn, :index, project.id, %{"archived" => "true"}))
    ...
    test "filters active", %{conn: conn, user: user} do
      ...
      conn =
        get(conn, project_questionnaire_path(conn, :index, project.id, %{"archived" => "false"}))
  ...
  describe "show:" do
    ...
    test "forbid access to questionnaire if the project does not belong to the current user", %{
      conn: conn
    } do
      questionnaire = insert(:questionnaire)
  ...
  describe "create:" do
    ...
    test "does not create resource and renders errors when data is invalid", %{
      ...
      conn =
        post conn, project_questionnaire_path(conn, :create, project.id),
          questionnaire: @invalid_attrs

      assert json_response(conn, 422)["errors"] != %{}
    end

    test "forbids creation of questionnaire for a project that belongs to another user", %{
      conn: conn
    } do
    ...
    test "creates and recreates variables", %{conn: conn, user: user} do
      ...
      conn =
        post original_conn, project_questionnaire_path(conn, :create, project.id),
      ...
      conn =
        put original_conn,
            project_questionnaire_path(original_conn, :update, project, questionnaire),
            questionnaire: %{steps: steps, settings: %{}}
  ...
  describe "update:" do
    ...
    test "rejects update if the questionnaire doesn't belong to the current user", %{conn: conn} do
      ...
      assert_error_sent :forbidden, fn ->
        put conn, project_questionnaire_path(conn, :update, questionnaire.project, questionnaire),
          questionnaire: @invalid_attrs
      end
    end
    ...
    test "rejects update for a snaphost", %{conn: conn, user: user} do
      ...
      assert_error_sent :not_found, fn ->
        put conn, project_questionnaire_path(conn, :update, questionnaire.project, snapshot),
          questionnaire: @valid_attrs
      end
    end
    ...
    test "updates and creates variables", %{conn: conn, user: user} do
      ...
      conn =
        put conn, project_questionnaire_path(conn, :update, project, questionnaire),
          questionnaire: %{steps: @dummy_steps, settings: %{}}
  ...
  describe "update translations" do
    test "creates no translations", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: %{steps: steps, quota_completed_steps: [], settings: %{}}
    ...
    test "creates translations for one sms prompt", %{conn: conn, user: user} do
      ...
      conn =
        put conn, project_questionnaire_path(conn, :update, project, questionnaire),
          questionnaire: %{steps: steps, quota_completed_steps: nil, settings: %{}}
    ...
    test "creates and recreates translations for other pieces", %{conn: conn, user: user} do
        ...
        multiple_choice_step(
          ...
          prompt: %{
            "en" => %{
              "sms" => "EN 1",
              "mobileweb" => "EN 3",
              "ivr" => %{"text" => "EN 2", "audio_source" => "tts"}
            },
        ...
        %{
          ...
          "prompt" => %{
            "en" => %{
              "sms" => "EN 5",
              "mobileweb" => "EN 7",
              "ivr" => %{"text" => "EN 6", "audio_source" => "tts"}
            },
      ...
      translations =
        ...
        |> Repo.all()
        |> Enum.map(
          &{&1.mode, &1.scope, &1.source_lang, &1.source_text, &1.target_lang, &1.target_text}
        )
        |> Enum.sort()
      ...
      steps = [
        multiple_choice_step(
          ...
          prompt: %{
            ...
            "fr" => %{
              "sms" => "",
              "ivr" => %{"text" => "FR 2 (NEW)", "audio_source" => "tts"},
              "mobileweb" => "FR 11"
            }
          },
          store: "X",
      ...
      conn =
        put original_conn, project_questionnaire_path(conn, :update, project, questionnaire),
          questionnaire: %{
            steps: steps,
            quota_completed_steps: quota_completed_steps,
            settings: %{}
  ...
  describe "delete:" do
    ...
    test "rejects delete if the questionnaire doesn't belong to the current user", %{conn: conn} do
      ...
      assert_error_sent :forbidden, fn ->
        delete(
          conn,
          project_questionnaire_path(conn, :delete, questionnaire.project, questionnaire)
        )
      end
    end
  ...
  describe "activity logs" do
    ...
    test "generates rename log if name changed", %{conn: conn, user: user} do
      ...
      assert_questionnaire_log(%{
        ...
        remote_ip: "192.168.0.128",
        metadata: %{
          "old_questionnaire_name" => questionnaire.name,
          "new_questionnaire_name" => "some content"
        }
      })
    end
    ...
    test "generates log for changes in modes", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: Map.merge(@valid_attrs, %{modes: ["sms", "mobileweb"]})
      ...
      assert_questionnaire_log(%{
        ...
        action: "add_mode",
        remote_ip: "192.168.0.128",
        metadata: %{"questionnaire_name" => "some content", "mode" => "mobileweb"}
      })
      ...
      assert_questionnaire_log(%{
        ...
        action: "remove_mode",
        remote_ip: "192.168.0.128",
        metadata: %{"questionnaire_name" => "some content", "mode" => "ivr"}
      })
    end
    ...
    test "generates log for changes in languages", %{conn: conn, user: user} do
      ...
      conn =
        put conn, project_questionnaire_path(conn, :update, project, questionnaire),
          questionnaire: Map.merge(@valid_attrs, %{languages: ["es", "en"]})
      ...
      assert_questionnaire_log(%{
        ...
        action: "add_language",
        remote_ip: "192.168.0.128",
        metadata: %{"questionnaire_name" => "some content", "language" => "es"}
      })
      ...
      assert_questionnaire_log(%{
        ...
        remote_ip: "192.168.0.128",
        metadata: %{"questionnaire_name" => "some content", "language" => "fr"}
      })
    end
    ...
    test "generates log for changes in quota completed steps", %{conn: conn, user: user} do
      ...
      log_1 =
        ...
        |> Repo.all()
        |> Enum.find(fn x ->
          x.action == "edit_step" && x.metadata["step_id"] == "quota-completed-step"
        end)
      ...
      assert_questionnaire_log(%{
        ...
        remote_ip: "192.168.0.128",
        metadata: %{
          "questionnaire_name" => "some content",
          "step_id" => "quota-completed-step",
    ...
    test "generates log when two steps are created", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: Map.merge(@valid_attrs, %{steps: all_steps})
      ...
      log_1 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "create_step" && x.metadata["step_id"] == first_step["id"]
        end)

      log_2 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "create_step" && x.metadata["step_id"] == second_step["id"]
        end)
      ...
      assert_questionnaire_log(%{
        ...
        metadata: %{
          ...
          "step_title" => second_step["title"],
          "step_type" => second_step["type"]
        }
      })
    ...
    test "generates log when two steps are deleted", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: Map.merge(@valid_attrs, %{steps: tail_steps})
      ...
      log_1 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "delete_step" && x.metadata["step_id"] == first_step["id"]
        end)

      log_2 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "delete_step" && x.metadata["step_id"] == second_step["id"]
        end)
    ...
    test "generates log when two steps are renamed", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: %{@valid_attrs | steps: [new_first_step, new_second_step | tail_steps]}
      ...
      log_1 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "rename_step" && x.metadata["step_id"] == first_step["id"]
        end)

      log_2 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "rename_step" && x.metadata["step_id"] == second_step["id"]
        end)
      ...
      assert_questionnaire_log(%{
        ...
        metadata: %{
          "questionnaire_name" => "some content",
          "step_id" => first_step["id"],
          "old_step_title" => first_step["title"],
          "new_step_title" => new_first_step["title"]
        }
      })
      ...
      assert_questionnaire_log(%{
        ...
        metadata: %{
          "questionnaire_name" => "some content",
          "step_id" => second_step["id"],
          "old_step_title" => second_step["title"],
          "new_step_title" => new_second_step["title"]
        }
      })
    ...
    test "generates log when a step is renamed and other is deleted", %{conn: conn, user: user} do
      ...
      put conn, project_questionnaire_path(conn, :update, project, questionnaire),
        questionnaire: %{@valid_attrs | steps: [new_first_step | tail_steps]}
    ...
    test "generates log when two steps are edited", %{conn: conn, user: user} do
      ...
      log_1 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "edit_step" && x.metadata["step_id"] == first_step["id"]
        end)

      log_2 =
        all_logs
        |> Enum.find(fn x ->
          x.action == "edit_step" && x.metadata["step_id"] == second_step["id"]
        end)
      ...
      assert_questionnaire_log(%{
        ...
        metadata: %{
          "questionnaire_name" => "some content",
          "step_id" => second_step["id"],
          "step_title" => new_second_step["title"]
        }
      })
