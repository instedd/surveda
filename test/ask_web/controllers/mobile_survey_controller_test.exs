defmodule AskWeb.MobileSurveyControllerTest do
  ...
  describe "index" do
    setup %{conn: conn} do
      ...
      quiz =
        insert(:questionnaire,
          steps: @mobileweb_dummy_steps,
          settings: %{
            "error_message" => %{"en" => %{"mobileweb" => "Invalid value"}},
            "title" => %{"en" => "Survey"},
            "mobile_web_intro_message" => "My HTML escaped & intro message",
            "mobile_web_color_style" => %{"secondary_color" => "#ae1", "primary_color" => "#e21"}
          }
        )
    ...
    test "includes config in root", %{conn: conn, respondent: respondent} do
      conn =
        get(
          conn,
          mobile_survey_path(conn, :index, respondent.id, %{
  ...
  test "using an invalid token", %{conn: conn} do
    ...
    conn =
      get(
        conn,
        mobile_survey_path(conn, :index, respondent.id, %{token: Respondent.token(respondent.id)})
      )
  ...
  test "respondent flow via mobileweb when respondent state is not active nor pending", %{
    conn: conn
  } do
    test_channel = TestChannel.new(false, true)
    ...
    quiz =
      insert(:questionnaire,
        steps: @mobileweb_dummy_steps,
        settings: %{"survey_already_taken_message" => %{"en" => "Already took this"}}
      )
  ...
  test "respondent flow via mobileweb when survey is over", %{conn: conn} do
    ...
    quiz =
      insert(:questionnaire,
        steps: @mobileweb_dummy_steps,
        settings: %{"mobile_web_survey_is_over_message" => "Bye"}
      )
  ...
  test "respondent flow via mobileweb with refusal + end", %{conn: conn} do
    ...
    quiz =
      insert(:questionnaire,
        steps: @mobileweb_refusal_dummy_steps,
        settings: %{"error_message" => %{"en" => %{"mobileweb" => "Invalid value"}}}
      )
