defmodule AskWeb.FloipControllerTest do
  ...
  describe "show" do
    test "does not leak packages that dont belong to user", %{conn: conn, user: _user} do
      ...
      assert_error_sent :forbidden, fn ->
        get(
          conn,
          project_survey_package_descriptor_path(
            conn,
            :show,
            project.id,
            survey.id,
    ...
    test "ensures provided package id is correct", %{conn: conn, user: user} do
      ...
      assert_error_sent :forbidden, fn ->
        get(
          conn,
          project_survey_package_descriptor_path(conn, :show, project.id, survey.id, "bar")
        )
      end
    end

    test "ensures provided package id belongs to running or terminated survey", %{
      conn: conn,
      user: user
    } do
      project = create_project_for_user(user)
      ...
      assert_error_sent :forbidden, fn ->
        get(
          conn,
          project_survey_package_descriptor_path(conn, :show, project.id, survey.id, "foo")
        )
      end
    end
  ...
  describe "responses" do
    test "injects a self referential link to the view", %{conn: conn, user: user} do
      ...
      requested_path =
        "#{project_survey_package_responses_path(conn, :responses, project.id, survey.id, "foo")}?foo=bar"

      conn = get(conn, requested_path)

      corresponding_descriptor_url =
        project_survey_package_descriptor_url(conn, :show, project.id, survey.id, "foo")
    ...
    test "injects links for next page and previous page", %{conn: conn, user: user} do
      ...
      base_path =
        project_survey_package_responses_path(conn, :responses, project.id, survey.id, "foo")
      ...
      full_query_params =
        "#{base_query_params}#{start_timestamp_filter}#{end_timestamp_filter}#{page_size}"

      conn = get(conn, "#{base_path}#{full_query_params}")

      base_url =
        project_survey_package_responses_url(conn, :responses, project.id, survey.id, "foo")
    ...
    test "ensures provided package id is correct", %{conn: conn, user: user} do
      ...
      assert_error_sent :forbidden, fn ->
        get(
          conn,
          project_survey_package_responses_path(conn, :responses, project.id, survey.id, "bar")
        )
      end
    end
