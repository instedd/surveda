defmodule Ask.Survey do
  ...
  defp retries_histogram_actives(%Survey{} = survey, stats, mode, now) do
    ...
    flow
      |> absolute_delay_flow()
      |> Stream.with_index()
      |> Enum.reduce([], fn {%{absolute_delay: absolute_delay, delay: delay, type: type}, idx}, acc ->
        acc ++ attempt_respondents(%{stats: stats, mode: mode, attempt: %{idx: idx + 1, absolute_delay: absolute_delay, current_mode: type, delay: delay}, now: now, attempts: Enum.count(flow), flow: flow})
      end)
  end

  defp attempt_respondents(%{stats: stats, mode: mode, attempt: %{idx: idx, absolute_delay: absolute_delay, delay: delay, current_mode: current_mode}, now: now, attempts: attempts, flow: flow}) do
    count_current = count_actives(%{stats: stats, attempt: idx, mode: mode, current_mode: current_mode})
    %{type: previous_mode} = flow |> Enum.at(idx - 2)
    count_previous = count_previous_actives(%{stats: stats, now: now, attempt: idx - 1, mode: mode, delay: delay, current_mode: current_mode, previous_mode: previous_mode})

    [%{hour: absolute_delay, respondents: count_current}] ++
    [%{hour: absolute_delay - delay, respondents: count_previous}] ++
    wating_respondents(%{stats: stats, mode: mode, attempt: %{idx: idx, absolute_delay: absolute_delay, delay: delay - 1, current_mode: current_mode}, now: now, attempts: attempts})
  end

  defp attempt_respondents(%{attempt: %{idx: 1}}), do: []

  defp count_previous_actives(%{previous_mode: "ivr"}), do:
    0

  defp count_previous_actives(%{stats: stats, now: now, attempt: attempt, mode: mode, delay: delay}), do:
    count_respondents(stats, now, attempt, mode, delay)

  defp count_previous_actives(_), do: 0

  defp count_actives(%{stats: stats, attempt: attempt, mode: mode, current_mode: "ivr"}), do:
    stats
      |> RetryStat.count(%{attempt: attempt, mode: mode, retry_time: ""})
  ...
  defp wating_respondents(%{stats: stats, mode: mode, attempt: %{idx: idx, absolute_delay: absolute_delay, delay: delay, current_mode: current_mode}, now: now, attempts: attempts}) when delay > 1 or (delay == 1 and attempts == idx and current_mode == "end") do
  ...
  defp wating_respondents(%{stats: stats, mode: mode, attempt: %{idx: idx, absolute_delay: absolute_delay, delay: delay}, now: now}) do
