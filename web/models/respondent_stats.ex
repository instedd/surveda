defmodule Ask.RespondentStats do
    ...
    group =
      ...
      |> Enum.map(fn field ->
        case @nilable_fields[field] do
          nil ->
            quote(do: field(s, unquote(field)))

          nil_value ->
            quote(do: fragment("NULLIF(?, ?)", field(s, unquote(field)), unquote(nil_value)))
        end
      end)
    ...
    quoted =
      case value do
        nil ->
          case @nilable_fields[field] do
            nil ->
              raise "Field #{field} is not nilable"
  ...
  def respondents_by_state(survey) do
    by_state_defaults =
      Ecto.Enum.values(Ask.Respondent, :state)
