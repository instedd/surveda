...
export const validate = (state: DataStore<Questionnaire>) => {
  ...
  validateMobileWebRequiredField(
    data,
    context,
    "mobileWebIntroMessage",
    k("Intro message must not be blank")
  )
  validateMobileWebRequiredField(
    data,
    context,
    "mobileWebSmsMessage",
    k("SMS message must not be blank")
  )
  validateMobileWebRequiredField(
    data,
    context,
    "mobileWebSurveyIsOverMessage",
    k('"Survey is over" must not be blank')
  )
  validateMobileWebRequiredField(
    data,
    context,
    "title",
    k("Title must not be blank"),
    "multilingual"
  )
  validateMobileWebRequiredField(
    data,
    ...
    k('"Survey already taken" message must not be blank'),
    "multilingual"
  )
...
const validateRefusal = (step: NumericStep, stepIndex, context, steps, path) => {
  if (step.refusal && step.refusal.enabled) {
    validateChoiceOrRefusal(
      step.refusal,
      context,
      stepIndex,
      stepIndex,
      steps,
      `${path}.refusal[refusal]`
    )
  }
}
...
const validateMobileWebLangPrompt = (
  prompt: Prompt,
  context: ValidationContext,
  lang: ?string,
  path: string
  ...
  if (isBlank(prompt.mobileweb)) {
    addError(
      context,
      `${path}.mobileweb`,
      k("Mobile web prompt must not be blank"),
      lang,
      "mobileweb"
    )
  }
...
const validateRangeDelimiters = (step, context, path) => {
  ...
  for (const delimiter of delimiters) {
    ...
    if (isNaN(int)) {
      addError(
        context,
        `${path}.rangesDelimiters`,
        k('Delimiter "{{delimiter}}" must be a number', { delimiter })
      )
    }
    ...
    if (previous == null && step.minValue != null && step.minValue > int) {
      addError(
        context,
        `${path}.minValue`,
        k("Min value must be less than or equal to the first delimiter ({{first}})", { first: int })
      )
    }
    ...
    if (previous != null && int <= previous) {
      addError(
        context,
        `${path}.rangesDelimiters`,
        k("Delimiter {{delimiter}} must be greater than the previous one ({{previous}})", {
          delimiter,
          previous,
        })
      )
  ...
  if (previous != null && step.maxValue != null && step.maxValue < previous) {
    addError(
      context,
      `${path}.maxValue`,
      k("Max value must be greater than or equal to the last delimiter ({{last}})", {
        last: previous,
      })
    )
  }
...
const validateChoices = (
  ...
  for (let i = 0; i < choices.length; i++) {
        ...
        for (let choiceIvr of choice.responses.ivr) {
          if (ivr.includes(choiceIvr)) {
            addError(
              context,
              `${choicePath}.ivr`,
              k('Value "{{value}}" already used in a previous response', {
                value: choiceIvr,
              }),
              null,
              "ivr"
    ...
    if (context.mobileweb) {
      context.languages.forEach((lang) =>
        validateMobileWebResponseDuplicates(
          choice,
          context,
          stepIndex,
          i,
...
const validateSmsResponseDuplicates = (
  choice: Choice,
  context: ValidationContext,
  stepIndex: number,
  choiceIndex: number,
...
const validateMobileWebResponseDuplicates = (
  choice: Choice,
  context: ValidationContext,
  stepIndex: number,
  choiceIndex: number,
...
const validateChoiceSmsResponse = (
  choice,
  context,
  stepIndex: number,
  choiceIndex: number,
...
const validateChoiceMobileWebResponse = (
  choice,
  context,
  stepIndex: number,
  choiceIndex: number,
...
const validateChoiceIvrResponse = (
  choice,
  context,
  stepIndex: number,
  choiceIndex: number,
  ...
  if (choice.responses.ivr && choice.responses.ivr.length == 0) {
    addError(context, path, k('"Phone call" must not be blank'), null, "ivr")
  }

  if (choice.responses.ivr && choice.responses.ivr.some((value) => !value.match("^[0-9#*]*$"))) {
    addError(
      context,
      path,
      k('"Phone call" must only consist of single digits, "#" or "*"'),
      null,
      "ivr"
    )
  }
...
const validateChoiceOrRefusal = (
  choice: Choice | Refusal,
  context: ValidationContext,
  stepIndex: number,
  choiceIndex: number,
  steps,
...
const validatePartialRelevantMinRelevantSteps = (
  data: Questionnaire,
  context: ValidationContext
) => {
  ...
  if (partialRelevantEnabled) {
    ...
    if (!minRelevantSteps) {
      addError(
        context,
        "partialRelevant.minRelevantSteps",
        k("Min relevant steps must not be blank")
      )
    } else if (minRelevantSteps > relevantStepsQuantity) {
      addError(
        context,
        "partialRelevant.minRelevantSteps",
        k("Min relevant steps must not be greater than the total amount of relevant steps")
      )
    }
  }
}

const validateMobileWebRequiredField = (
  data: Questionnaire,
  context: ValidationContext,
  fieldName: string,
...
const validateMobileWebColorStyle = (data, context) => {
  ...
  if (primary) {
    if (!colorRegex.test(primary)) {
      addError(
        context,
        "mobileWebColorStyle.primaryColor",
        k("Mobile web primary color is invalid "),
        null,
        "mobileweb"
      )
    }
  ...
  if (secondary) {
    if (!colorRegex.test(secondary)) {
      addError(
        context,
        "mobileWebColorStyle.secondaryColor",
        k("Mobile web secondary color is invalid"),
        null,
        "mobileweb"
      )
    }
