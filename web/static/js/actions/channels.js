...
export const fetchProjectChannels =
  (projectId: number) => (dispatch: Function, getState: () => Store) => {
    const state = getState()

    // Don't fetch channels if they are already being fetched
    if (
      state.channels.fetching &&
      state.channels.filter &&
      state.channels.filter.projectId == projectId
    ) {
      return Promise.resolve(getState().channels.items)
    }
...
export const createChannel =
  (providerType: string, baseUrl: string, channel: Object) => (dispatch: Function) => {
    return api.createChannel(providerType, baseUrl, channel).then(() => dispatch(fetchChannels()))
  }
