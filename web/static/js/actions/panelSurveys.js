...
export const fetchPanelSurveys =
  (projectId: number) =>
  (dispatch: Function, getState: () => Store): Promise<?SurveyList> => {
    ...
    if (
      state.panelSurveys.fetching &&
      state.panelSurveys.filter &&
      state.panelSurveys.filter.projectId == projectId
    ) {
    ...
    return api
      ...
      .then((response) =>
        dispatch(receivePanelSurveys(projectId, response.entities.panelSurveys || {}))
      )
...
export const receivePanelSurveys = (
  projectId: number,
  items: IndexedList<PanelSurvey>
