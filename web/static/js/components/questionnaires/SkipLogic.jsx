...
class SkipLogic extends Component<Props, State> {
    ...
    if (
      ...
      skipOptions.unshift({
        ...
        title:
          stepIndex == -1
            ? t("Step missing")
            : this.stringOrUntitledQuestion(stepsBefore[stepIndex].title),
        enabled: false,
      })
    }
  ...
  render() {
    ...
    let { skipOptions, currentValueIsValid } = this.skipOptions(
      this.state.value,
      stepsAfter,
      stepsBefore
    )
    ...
    return (
      <select
        ...
        className={classNames("browser-default", {
          invalidValue: !currentValueIsValid,
        })}
      >
