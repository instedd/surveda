...
export default class RetriesHistogram extends Component<Props> {
  ...
  calculateSize() {
    ...
    const flow = this.props.flow.map((step) => ({
      ...step,
      delay: Math.ceil(step.delay),
    }))
    let width = 1
    ...
    const yCompletes = d3
      .scaleLinear()
      .domain([d3.max(completes, (d) => d.value), 0])
      .range([0, completesHeight])
    const x = d3
      .scaleBand()
      .domain(d3.range(0, d3.sum(flow, (step) => step.delay) + 1, 1))
      .rangeRound([0, width])
      .padding(0.1)
    ...
    d3.select(this.refs.axis)
      .call(
        d3
          .axisLeft(yActives)
          .tickSizeInner(0)
          .tickValues(yActives.ticks(3).filter((tick) => Number.isInteger(tick)))
    ...
    d3.select(this.refs.grid)
      .call(
        d3
          ...
          .ticks(3)
          .tickValues(yActives.ticks(3).filter((tick) => Number.isInteger(tick)))
          .tickSizeInner(actives.length * x.step() - 1)
      )
      .selectAll("text")
      .remove()
  }
      ...
      <g>
        <path
          ...
          d={`M${-padding} 0
                                      ...
                                      z`}
        />
        <text x={-width / 2} y={-6} className="label">
          {label}
        </text>
      </g>
    )
    ...
    return (
      <div className="retriesHistogram" ref="container">
        ...
        <svg
          ...
          height={activesHeight + completesHeight + margin.top + margin.bottom + padding}
        >
          <g transform={`translate(${margin.left},${margin.top})`}>
            <g ref="actives">
              {actives.map((slot, index) => {
                let isFix = slot.value === fix
                let isDiscard = flow.some(
                  (step) => step.offset === index && step.type === "discard"
                )
                let isTrying = flow.some(
                  (step) => step.offset === index && timewindows[step.offset]
                )
                let className =
                  "bar " + (isFix ? "fix" : isDiscard ? "red" : isTrying ? "trying" : "standby")
                ...
                return (
                  <rect
                    ...
                    height={
                      slot.value === fix ? activesHeight : activesHeight - yActives(slot.value)
                    }
            ...
            <g ref="timewindows">
              <path
                fillRule="evenodd"
                className="out"
                d={this.rect(x(0), 0, timewindows.length * x.step(), activesHeight).concat(
                  timewindows
              ...
              {flow.map((step, index) => {
                ...
                return (
                  <g key={index} transform={`translate(${x(step.offset)},0)`}>
                    ...
                    <path
                      className={`dottedLine ${state}`}
                      transform={`translate(0, ${margin.top / 2 - padding})`}
                      d={`M0 0v${activesHeight + padding * 2}`}
                    />
                  </g>
                )
              })}
        ...
        <div className="bottom">
          <div className="status">
            {scheduleDescription ? <span className="icon">access_time</span> : null}
            {`${scheduleDescription}`}
          </div>
