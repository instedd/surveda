...
class SurveyForm extends Component {
  ...
  launchSurvey() {
    const { projectId, survey, router, dispatch } = this.props
    launchSurvey(projectId, survey.id).then(() => {
  ...
  render() {
    ...
    const questionnaireStepCompleted =
      survey.questionnaireIds != null &&
      survey.questionnaireIds.length > 0 &&
      this.questionnairesValid(survey.questionnaireIds, questionnaires)
    const respondentsStepCompleted =
      respondentGroups &&
      Object.keys(respondentGroups).length > 0 &&
      every(values(respondentGroups), (group) => {
        return group.channels.length > 0 && this.allModesHaveAChannel(survey.mode, group.channels)
      })

    const modeStepCompleted =
      survey.mode != null &&
      survey.mode.length > 0 &&
      this.questionnairesMatchModes(survey.mode, survey.questionnaireIds, questionnaires)
    const cutoffStepCompleted = cutOffConfigValid && questionnaireStepCompleted
    const validRetryConfiguration =
      !errors ||
      (!errors.smsRetryConfiguration && !errors.ivrRetryConfiguration && !errors.fallbackDelay)
    ...
    let mandatorySteps = [
      questionnaireStepCompleted,
      modeStepCompleted,
      respondentsStepCompleted,
    ...
    const allStepsCompleted =
      mandatorySteps.filter((item) => item == true).length == mandatorySteps.length
      ...
      launchComponent = (
        <Tooltip text={t("Launch survey")}>
          <a
            ...
            style={{ top: "90px", left: "-5%" }}
            onClick={() => this.launchSurvey()}
          >
    ...
    const cutoffRulesStep = () => (
      <div>
        <div id="cutoff" className="row scrollspy">
          <SurveyWizardCutoffStep
            survey={survey}
            questionnaire={questionnaire}
            readOnly={readOnly || surveyStarted}
          />
      ...
      <div className="row">
          ...
          <PositionFixer offset={60}>
            <ul className="collection with-header wizard">
              ...
              <CollectionItem
                path="#questionnaire"
                icon="assignment"
                text={t("Select a questionnaire")}
                completed={!!questionnaireStepCompleted}
              />
              <CollectionItem
                path="#channels"
                icon="settings_input_antenna"
                text={t("Select mode")}
                completed={!!modeStepCompleted}
              />
              <CollectionItem
                path="#respondents"
                icon="group"
                text={t("Upload your respondents list")}
                completed={!!respondentsStepCompleted}
              />
              <CollectionItem
                path="#schedule"
                icon="today"
                text={t("Setup a schedule")}
                completed={!!scheduleStepCompleted}
              />
              {includeCutoffRulesStep ? (
                <CollectionItem
                  path="#cutoff"
                  icon="remove_circle"
                  text={t("Setup cutoff rules")}
                  completed={!!cutoffStepCompleted}
                />
              ) : null}
              ...
              {survey.comparisons.length > 0 ? (
                <CollectionItem
                  ...
                  icon="call_split"
                  text={t("Comparisons")}
                  completed={!!comparisonsStepCompleted}
                />
              ) : (
                ""
              )}
        ...
        <div className="col s12 m7 offset-m1 wizard-content">
          ...
          <div id="channels" className="row scrollspy">
            <SurveyWizardModeStep
              survey={survey}
              questionnaires={questionnaires}
              readOnly={readOnly || surveyStarted}
              respondentGroups={respondentGroups}
            />
            <ScrollToLink target="#respondents">
              {t("NEXT: Upload your respondents list")}
            </ScrollToLink>
          </div>
