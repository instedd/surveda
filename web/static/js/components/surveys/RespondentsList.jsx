...
export const RespondentsList = translate()(
  ({
    survey,
    group,
    add,
    ...
    if (add || remove) {
      footer = (
        <div className="card-action">
          <div className="row">
            <div className="col s6 left-align">{add}</div>
            <div className="col s6 right-align">{remove}</div>
          </div>
        </div>
      )
    }
    ...
    for (const targetMode of modes) {
      channelsComponent.push(
        NewRespondentChannel(
          targetMode,
          channels,
          currentChannels,
          onChannelChange,
            ...
            <div className="col s6">
              <table className="ncdtable">
                <thead>
                  <tr>
                    <th>
                      {t("{{groupName}} ({{count}} contact)", {
                        groupName: group.name,
                        count: group.respondentsCount,
                      })}
                    </th>
                  </tr>
                </thead>
