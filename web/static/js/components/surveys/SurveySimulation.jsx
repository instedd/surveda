...
class SurveySimulation extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(actions.fetchSurveyIfNeeded(projectId, surveyId)).then(() => {
  ...
  loadInitialStateIfNeeded() {
    ...
    api.fetchSurveySimulationInitialState(projectId, surveyId, mode).then((initialState) => {
  ...
  refreshStatus() {
    ...
    api.fetchSurveySimulationStatus(projectId, surveyId).then(
      (status) => {
  ...
  stopSimulation(e) {
    ...
    stopSimulationModal.open({
      ...
      onConfirm: () => {
        api.stopSurveySimulation(projectId, surveyId).then((response) => {
  ...
  stepsComponent() {
    ...
    if (hasSections(questionnaire.steps)) {
      stepList = flatten(
        questionnaire.steps.map((step, index) => this.stepsComponentWhenSections(step, index))
      )
    } else {
    ...
    return (
      <div className="quex-simulation-steps">
        <Card>
          <ul className="collection simulation">{stepList}</ul>
        </Card>
      </div>
    )
  }
  ...
  mobileWebSimulation() {
    ...
    return (
      <div className="quex-simulation-container">
        ...
        <div>{this.stepsComponent()}</div>
        {mobileWebUrl ? (
          <div className="mobile-web-iframe-container">
            <div className="chat-header">
              <button
                onClick={() => {
                  this.setState({ mobileWebUrl: null })
                }}
              >
          ...
          <div
            onClick={(event) => this.onChatWindowClick(event)}
      ...
      <div>
        <Tooltip text={t("Stop simulation")}>
          <a
            ...
            onClick={(e) => this.stopSimulation(e)}
          >
            <i className="material-icons">clear</i>
          </a>
        </Tooltip>
        <ConfirmationModal
          modalId="survey_stop_simulation_modal"
          ref="stopSimulationModal"
          confirmationText={t("Stop")}
          header={t("Stop simulation")}
          showCancel
        />
        <div className="row">
          ...
          <div className="col s12 m8">{this.stepsComponent()}</div>
        </div>
      </div>
    )
  }
