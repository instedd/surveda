...
class DownChannelsStatus extends PureComponent {
  ...
  downChannelsDescription(channelNames, timestamp) {
    ...
    if (channelNames.length > 1) {
      return <Trans>Channels <em>{{names}}</em> down <TimeAgo date={timestamp} formatter={this.bindedDownChannelsFormatter} /></Trans>
    } else {
      const name = channelNames[0]
      return <Trans>Channel <em>{{name}}</em> down <TimeAgo date={timestamp} formatter={this.bindedDownChannelsFormatter} /></Trans>
    }
  }
}
