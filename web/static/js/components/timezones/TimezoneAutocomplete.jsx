...
class TimezoneAutocomplete extends Component {
  ...
  autocompleteGetData(value, callback) {
    ...
    const matches = (value, tz) => {
      return tz.indexOf(value) !== -1 || tz.toLowerCase().indexOf(value) !== -1
    }
    const matchingOptions = Object.keys(timezones.items).filter((tz) =>
      matches(formatTimezone(value.toLowerCase()), formatTimezone(tz))
    )
  ...
  render() {
    ...
    if (!timezones || !timezones.items) {
      return <div>{t("Loading timezones...")}</div>
    } else {
        ...
        <div className="input-field timezone-selection">
          <InputWithLabel value={this.state.timezone.text} label={t("Timezone")}>
            <input
              type="text"
              ref={(ref) => {
                this.timeZoneInput = ref
              }}
              id="timeZoneInput"
              autoComplete="off"
              className="timezone-input autocomplete"
          ...
          <Autocomplete
            getInput={() => {
              return this.getTimezoneInputRef()
            }}
          ...
          {timeAtSelectedTz && (
            <span className="small-text-bellow">
              {t("Time at selected timezone: {{time}}", {
                time: timeAtSelectedTz,
              })}
            </span>
          )}
        </div>
