...
export const fetchSurvey =
  ...
  (dispatch: Function, getState: () => Store): Survey => {
    dispatch(fetch(projectId, id))
    return api
      .fetchSurvey(projectId, id)
      .then((response) => {
...
export const fetchSurveyRetriesHistograms =
  (projectId: number, id: number) => (dispatch: Function) => {
    api
      .fetchSurveyRetriesHistograms(projectId, id)
...
export const fetchSurveyIfNeeded =
  (projectId: number, id: number) =>
  (dispatch: Function, getState: () => Store): Promise<?Survey> => {
...
export const changeFolder =
  (survey: Survey, folderId: number) => (dispatch: Function, getState: () => Store) => {
    return api
      .setFolderId(survey.projectId, survey.id, folderId)
      .then(() => dispatch(surveysActions.folderChanged(survey.id, folderId)))
  }

export const changeDescription =
  (newDescription: string) => (dispatch: Function, getState: () => Store) => {
    const survey = getState().survey.data
    if (!survey) return
...
export const deleteSurvey = (survey: Survey) => (dispatch: Function) => {
  api.deleteSurvey(survey.projectId, survey).then(() => dispatch(surveysActions.deleted(survey)))
}

export const deletePanelSurveyWave =
  (panelSurvey: PanelSurvey, survey: Survey) => (dispatch: Function) => {
    api.deleteSurvey(survey.projectId, survey).then(() => {
      ...
      if (panelSurvey.folderId) {
        ...
        // refresh the panel surveys (at root)
...
export const save = () => (dispatch: Function, getState: () => Store) => {
  ...
  return api
    ...
    .then((response) => dispatch(saved(response.entities.surveys[response.result])))
}
...
export const createIncentivesLink =
  (projectId: number, surveyId: number) => (dispatch: Function) => {
    api.createIncentivesLink(projectId, surveyId).then((response) => {
      return dispatch(receiveLink(response))
    })
  }

export const createInteractionsLink =
  (projectId: number, surveyId: number) => (dispatch: Function) => {
    api.createInteractionsLink(projectId, surveyId).then((response) => {
      return dispatch(receiveLink(response))
    })
  }
...
export const refreshResultsLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.refreshResultsLink(projectId, surveyId).then((response) => {
      return dispatch(refreshLink(response))
    })
  }

export const refreshIncentivesLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.refreshIncentivesLink(projectId, surveyId).then((response) => {
      return dispatch(refreshLink(response))
    })
  }

export const refreshInteractionsLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.refreshInteractionsLink(projectId, surveyId).then((response) => {
      return dispatch(refreshLink(response))
    })
  }
...
export const deleteResultsLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.deleteResultsLink(projectId, surveyId).then((response) => {
      return dispatch(deleteLink(link))
    })
  }

export const deleteIncentivesLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.deleteIncentivesLink(projectId, surveyId).then((response) => {
      return dispatch(deleteLink(link))
    })
  }
...
export const deleteDispositionHistoryLink =
  (projectId: number, surveyId: number, link: Link) => (dispatch: Function) => {
    api.deleteDispositionHistoryLink(projectId, surveyId).then((response) => {
      return dispatch(deleteLink(link))
    })
  }
