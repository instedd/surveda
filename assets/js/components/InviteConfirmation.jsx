...
class InviteConfirmation extends Component {
  componentDidMount() {
    ...
    dispatch(actions.fetchInvite(code)).then(
      (invite) => {
        if (invite.error) {
    ...
    return (
      <div className="row accept-invitation">
        <div className="col s12 center">
          <h1>
            <i className="material-icons grey-text xxlarge">folder_shared</i>
          </h1>
          <p>
          ...
          <a className="btn-medium blue" onClick={() => this.confirmInvitation()}>
            {t("Accept invitation")}
          </a>
        </div>
      </div>
