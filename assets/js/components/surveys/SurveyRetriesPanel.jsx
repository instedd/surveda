...
class SurveyRetriesPanel extends Component {
  ...
  render() {
    ...
    const getHistograms = () => {
      ...
      return retriesHistograms
        .map((h) => getHistogram(h))
        .map((h) => (
          <RetriesHistogram
      ...
      <div className="retries-histogram" style={{ marginTop: "20px" }}>
        <div className="header">
          ...
          <div className="description">
            {t("Number of contacts in each stage of the retry schedule")}
          </div>
        </div>
        {getHistograms()}
      </div>
...
const mapStateToProps = (state, ownProps) => {
  const surveyRetriesHistograms =
    ownProps.surveyId == state.surveyRetriesHistograms.surveyId
      ? state.surveyRetriesHistograms.histograms
      : null
