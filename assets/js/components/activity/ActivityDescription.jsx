...
class ActivityDescription extends Component {
  ...
  text(activity) {
    ...
    switch (activity.entityType) {
        ...
        switch (activity.action) {
          ...
          case "change_folder":
            ...
            if (oldFolderName) {
              ...
              return t(
                ...
                { surveyName: surveyName, newFolderName: newFolderName }
              )
            }
          case "rename":
            const { oldSurveyName, newSurveyName } = metadata
            if (oldSurveyName) {
              return t("Renamed <i>{{oldSurveyName}}</i> survey to <i>{{newSurveyName}}</i>", {
                oldSurveyName: oldSurveyName,
                newSurveyName: newSurveyName,
              })
            } else {
              return t("Named survey as <i>{{newSurveyName}}</i>", {
                newSurveyName: newSurveyName,
              })
            }
          case "change_description":
            ...
            if (oldSurveyDescription) {
              return t("Changed description of <i>{{surveyName}}</i> survey", {
                surveyName: surveyName,
              })
            } else {
              return t("Set description of <i>{{surveyName}}</i> survey", {
                surveyName: surveyName,
              })
            }
          case "edit":
            return t("Edited <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "delete":
            return t("Deleted <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "start":
            return t("Started <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "request_cancel":
          case "stop":
            return t("Stopped <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "completed_cancel":
            return t("Cancelled <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "lock":
            return t("Locked <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "unlock":
            return t("Unlocked <i>{{surveyName}}</i> survey", {
              surveyName: surveyName,
            })
          case "download":
            return t("Downloaded <i>{{surveyName}}</i> {{reportType}}", {
              surveyName: surveyName,
          ...
          case "disable_public_link":
            return t("Disabled <i>{{surveyName}}</i> {{reportType}} link", {
              surveyName: surveyName,
              reportType: reportType,
            })
          case "regenerate_public_link":
            return t("Reset <i>{{surveyName}}</i> {{reportType}} link", {
              surveyName: surveyName,
              reportType: reportType,
            })
          case "add_respondents":
            ...
            return t(
              ...
              { surveyName, respondentsCount, fileName }
            )
          default:
            return ""
        }
      case "project":
        switch (activity.action) {
          case "create_invite":
            return t("Invited {{collaboratorEmail}} as {{role}}", {
              collaboratorEmail: metadata["collaboratorEmail"],
              role: metadata["role"],
            })
          case "edit_invite":
            return t(
          ...
          case "delete_invite":
            return t("Deleted invitation for {{collaboratorEmail}} as {{role}}", {
              collaboratorEmail: metadata["collaboratorEmail"],
              role: metadata["role"],
            })
          case "edit_collaborator":
            return t(
              ...
              {
                collaboratorName: metadata["collaboratorName"],
                oldRole: metadata["oldRole"],
                newRole: metadata["newRole"],
              }
            )
          case "remove_collaborator":
            return t("Deleted {{role}} membership for {{collaboratorName}}", {
              collaboratorName: metadata["collaboratorName"],
              role: metadata["role"],
            })
          default:
      ...
      case "questionnaire":
        ...
        switch (activity.action) {
          ...
          case "delete":
            return t("Deleted questionnaire <i>{{questionnaireName}}</i>", {
              questionnaireName,
            })
          case "rename":
            ...
            if (oldQuestionnaireName) {
              return t(
                ...
                {
                  oldQuestionnaireName: oldQuestionnaireName,
                  newQuestionnaireName: newQuestionnaireName,
                }
              )
            } else {
              return t("Named questionnaire as <i>{{newQuestionnaireName}}</i>", {
                newQuestionnaireName: newQuestionnaireName,
              })
            }
          case "add_mode":
            return t("Added mode <i>{{mode}}</i> to <i>{{questionnaireName}}", {
              questionnaireName,
              mode: this.modeName(metadata["mode"]),
            })
          case "remove_mode":
            return t("Removed mode <i>{{mode}}</i> from <i>{{questionnaireName}}", {
              questionnaireName,
              mode: this.modeName(metadata["mode"]),
            })
          case "add_language":
          ...
          case "edit_step":
            return t("Edited step <i>{{stepTitle}}</i> of <i>{{questionnaireName}}</i>", {
              questionnaireName,
              stepTitle,
            })
          case "rename_step":
            ...
            return t(
              ...
              { questionnaireName, oldStepTitle, newStepTitle }
            )
          case "delete_step":
            return t("Removed step <i>{{stepTitle}}</i> from <i>{{questionnaireName}}</i>", {
              questionnaireName,
              stepTitle,
            })
          case "edit_settings":
            return t("Edited settings on <i>{{questionnaireName}}</i>", {
              questionnaireName,
            })
          case "create_section":
            return t("Added section <i>{{sectionTitle}}</i> to <i>{{questionnaireName}}</i>", {
              sectionTitle,
              questionnaireName,
            })
          case "delete_section":
            return t("Removed section <i>{{sectionTitle}}</i> from <i>{{questionnaireName}}</i>", {
              sectionTitle,
              questionnaireName,
            })
          case "edit_section":
            return t("Edited section <i>{{sectionTitle}}</i> of <i>{{questionnaireName}}</i>", {
              sectionTitle,
              questionnaireName,
            })
          case "rename_section":
            ...
            return t(
              ...
              { oldSectionTitle, questionnaireName, newSectionTitle }
            )
        }
        break
      case "folder":
        ...
        switch (activity.action) {
          ...
          case "rename":
            const { oldFolderName, newFolderName } = metadata
            return t("Renamed <i>{{oldFolderName}}</i> folder to <i>{{newFolderName}}</i>", {
              oldFolderName,
              newFolderName,
            })
        }
          ...
          case "change_folder":
            ...
            if (oldFolderName) {
              return t(
                ...
                {
                  panelSurveyName: panelSurveyName,
                  oldFolderName: oldFolderName,
                  newFolderName: newFolderName,
                }
              )
  ...
  render() {
    ...
    return <span dangerouslySetInnerHTML={{ __html: this.text(activity) }} />
  }
}
