...
class TestQuestionnaireModal extends Component {
  ...
  modeOptions() {
    ...
    if (questionnaire.modes.indexOf("mobileweb") != -1) {
      options.push(
        <option key="mobileweb" value="mobileweb">
          {t("Mobile web")}
        </option>
      )
    }
  ...
  render() {
    ...
    const activeButton = ({ onClick }) => (
      <a className="btn-large waves-effect waves-light blue modal-action" onClick={onClick}>
        Send
      </a>
    )
    let sendButton = null
    ...
    if (this.state.channelId == "simulation") {
      sendButton = activeButton({
        onClick: () => {
          this.refs.modal.close()
          router.push(
            routes.questionnaireSimulation(questionnaire.projectId, questionnaire.id, mode)
          )
        },
      })
      ...
      <Modal card id={modalId} ref="modal" className="hidden-overflow">
        <div className="modal-content">
          <div className="card-title header">
            ...
            <p>
              {t("Test this questionnaire on a real device, the collected data will be ignored")}
            </p>
          </div>
          <div className="card-content">
            <div className="row">
              ...
              <Input
                ...
                onChange={(e) => this.changeChannelId(e)}
                label={t("Channel")}
              >
...
export default translate()(
  withRouter(
    connect(mapStateToProps, mapDispatchToProps)(withQuestionnaire(TestQuestionnaireModal))
  )
)
