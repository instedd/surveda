...
class QuestionnaireClosedStep extends Component<Props> {
  ...
  render() {
    const { step, onClick, hasErrors, readOnly, quotaCompletedSteps, partialRelevantEnabled, t } =
      this.props
    ...
    const renderRelevant = (relevant) => (
      ...
      >
        <Tooltip
          text={
            relevant
              ? t("This question is relevant for partial flag")
              : t("This question is not relevant for partial flag")
          }
        >
          <i
            className={`material-icons ${relevant ? "green-text darken-2" : "grey-text darken-3"}`}
          >
            star
          </i>
        </Tooltip>
          ...
          <div className="card-content closed-step">
            <div>
              ...
              >
                ...
                <UntitledIfEmpty
                  className={classNames({ "text-error": hasErrors })}
                  text={step.title}
                  emptyText={t("Untitled question")}
                />
                <i
                ...
                {partialRelevantEnabled && canBeRelevant(step.type)
                  ? renderRelevant(step.relevant)
                  : null}
              </a>
            </div>
          </div>
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    hasErrors: hasErrorsInPrefixWithModeAndLanguage(
    ...
    partialRelevantEnabled:
      questionnaire &&
      questionnaire.partialRelevantConfig &&
      questionnaire.partialRelevantConfig.enabled,
  }
}
...
export default translate()(
  withQuestionnaire(connect(mapStateToProps, mapDispatchToProps)(QuestionnaireClosedStep))
)
