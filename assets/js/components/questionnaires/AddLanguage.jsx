...
class AddLanguage extends Component {
  ...
  autocompleteGetData(value, callback) {
    ...
    languagesOptions = languagesOptions.filter(
      (lang) =>
        (questionnaire.languages || []).indexOf(lang.id) == -1 &&
        questionnaire.defaultLanguage !== lang
    )

    const matchingOptions = languagesOptions.filter((lang) =>
      matchesLanguage(value.toLowerCase(), lang)
    )
      ...
      return (
        <div className="input-field language-selection">
          <input
            ...
            placeholder={t("Start typing a language")}
